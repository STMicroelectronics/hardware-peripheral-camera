...
MetadataReader::MetadataReader(
    std::unique_ptr<const helper::CameraMetadata> metadata)
...
int MetadataReader::Facing(int* facing) const {
  ...
  int res = MetadataCommon::SingleTagValue(*metadata_, ANDROID_LENS_FACING,
                                                              &metadata_facing);
  ...
  if (metadata_facing > ANDROID_LENS_FACING_EXTERNAL) {
    ALOGE("%s: Invalid facing from static metadata: %d.",
              __func__, metadata_facing);
    return -EINVAL;
  }

  *facing = metadata_facing;
...
int MetadataReader::Orientation(int* orientation) const {
  ...
  int res = MetadataCommon::SingleTagValue(*metadata_,
...
int MetadataReader::StreamConfigurations(
  ...
  // Convert from raw.
  configs->insert(configs->end(), raw_stream_configs.begin(),
                                                      raw_stream_configs.end());
  ...
  for (const auto& config : *configs) {
    ...
    if (config.spec.width < 1 || config.spec.height < 1) {
      ALOGE("%s: Invalid stream config: non-positive dimensions (%d, %d).",
                __func__, config.spec.width, config.spec.height);
      return -EINVAL;
    }
...
int MetadataReader::StreamStallDurations(
  ...
  int res =
      MetadataCommon::VectorTagValue(*metadata_,
  ...
  for (const auto& stall : *stalls) {
    ...
    if (stall.spec.width < 1 || stall.spec.height < 1) {
      ALOGE("%s: Invalid stall duration: non-positive dimensions (%d, %d).",
                __func__, stall.spec.width, stall.spec.height);
      return -EINVAL;
    }

    // Must have a non-negative stall.
    if (stall.duration < 0) {
      ALOGE("%s: Invalid stall duration: negative stall %" PRId64 ".",
                __func__, stall.duration);
      return -EINVAL;
    }
...
int MetadataReader::ReprocessFormats(ReprocessFormatMap* reprocess_map) const {
  ...
  for (size_t i = 0; i < input_output_formats.size();) {
    ...
    if (num_output_formats < 1) {
      ALOGE("%s: No output formats for input format %d.",
