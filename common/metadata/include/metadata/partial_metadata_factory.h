...
std::unique_ptr<Control<T>> NoEffectMenuControl(
  ...
  if (options.empty()) {
    ALOGE("%s: At least one option must be provided.", __FUNCTION__);
    return nullptr;
  }
...
std::unique_ptr<Control<T>> V4L2Control(
  ...
  if (res) {
    ALOGE("%s: Failed to query control %d.", __FUNCTION__, control_id);
    return nullptr;
  }
  ...
  if (control_min > control_max) {
    ALOGE("%s: No acceptable values (min %d is greater than max %d).",
              __FUNCTION__, control_min, control_max);
    return nullptr;
  }
  ...
  switch (control_query.type) {
    case V4L2_CTRL_TYPE_BOOLEAN:
      if (type != ControlType::kMenu) {
        ALOGE(
            "%s: V4L2 control %d is of type %d, which isn't compatible with "
      ...
      for (int32_t i = control_min; i <= control_max; i += control_step) {
        ...
        if (res == -EINVAL) {
          ALOGV("%s: V4L2 value %d for control %d has no metadata equivalent.",
                    __FUNCTION__, i, control_id);
          continue;
        } else if (res) {
          ALOGE("%s: Error converting value %d for control %d.",
                    __FUNCTION__, i, control_id);
          return nullptr;
        }
      ...
      if (options.empty()) {
        ALOGE("%s: No valid options for control %d.", __FUNCTION__, control_id);
        return nullptr;
      }
    ...
    case V4L2_CTRL_TYPE_INTEGER:
      ...
      if (res) {
        ALOGE(
          "%s: Failed to convert V4L2 min value %d for control %d to metadata.",
              __FUNCTION__,
              control_min,
              control_id);
        return nullptr;
      ...
      if (res) {
        ALOGE(
          "%s: Failed to convert V4L2 max value %d for control %d to metadata.",
              __FUNCTION__,
              control_max,
              control_id);
        return nullptr;
      ...
      break;
    default:
      ALOGE("%s: Control %d (%s) is of unsupported type %d",
              __FUNCTION__, control_id, control_query.name, control_query.type);
      return nullptr;
  }
