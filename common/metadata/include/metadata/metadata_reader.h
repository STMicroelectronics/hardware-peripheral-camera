...
class MetadataReader {
 public:
  MetadataReader(std::unique_ptr<const helper::CameraMetadata> metadata);
 ...
 private:
  std::unique_ptr<const helper::CameraMetadata> metadata_;

  MetadataReader(const MetadataReader&);
  void operator=(const MetadataReader&);
};
...
#endif  // DEFAULT_CAMERA_HAL_:METADATA_METADATA_READER_H_
